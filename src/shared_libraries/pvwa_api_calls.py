...
def create_account_on_vault(session, account_name, account_password, secret_type, store_parameters_class, platform_id, address,
    ...
    logger.debug(f"Creating account in vault for '{instance_id}'")
    ...
    if rest_response.status_code == requests.codes.created:
        logger.debug(f"Account for instance '{instance_id}' was successfully created in the vault")
        return True, ""
    logger.error(f"Failed to create the account for instance '{instance_id}' from the vault. status code:{rest_response.status_code}")
...
def rotate_credentials_immediately(session, pvwa_url, account_id, instance_id):
    ...
    logger.debug(f"Rotating '{instance_id}' credentials")
    header = DEFAULT_HEADER
    header.update({"Authorization": session})
    ...
    if rest_response.status_code == requests.codes.ok:
        logger.debug(f"Call for immediate key change for '{instance_id}' performed successfully")
        return True
...
def get_account_value(session, account, instance_id, rest_url):
    ...
    logger.debug(f"Getting '{instance_id}' account from vault")
    header = DEFAULT_HEADER
    header.update({"Authorization": session})
    ...
    if rest_response.status_code == requests.codes.ok:
        ...
        logger.debug(f"Account {account} for instance '{instance_id}', not found on vault")
        return False
...
def delete_account_from_vault(session, account_id, instance_id, pvwa_url):
    ...
    logger.debug(f"Deleting '{instance_id}' from vault")
    header = DEFAULT_HEADER
    header.update({"Authorization": session})
    rest_url = f"{pvwa_url}/api/Accounts/{account_id}"
    rest_response = pvwa_integration_class.call_rest_api_delete(rest_url, header)

    if rest_response.status_code != requests.codes.no_content:
        logger.trace(f'Status code {rest_response.status_code}', caller_name='delete_account_from_vault')
        if rest_response.status_code == requests.codes.not_found:
            logger.error(f"Failed to delete the account for instance '{instance_id}' from the vault. The account does not exists")
            raise Exception(f"Failed to delete the account for instance '{instance_id}' from the vault. The account does not exists")
        logger.info(f"Failed to delete the account for instance '{instance_id}' from the vault. An error occurred. Attempting to disable automatic management and marking for manual deletion")
        dataAddress = f"""
        [
            {{
            "op": "replace",
        ...
        if rest_response_dataAddress.status_code != requests.codes.ok:
            logger.error(f"Failed to delete and update the account for instance '{instance_id}' from the vault. Manual removal required")
...
def check_if_kp_exists(session, account_name, safe_name, instance_id, rest_url):
    ...
    logger.debug('Checking if key pair is onboarded')
    header = DEFAULT_HEADER
    header.update({"Authorization": session})
...
def retrieve_account_id_from_account_name(session, account_name, safe_name, instance_id, rest_url):
    ...
    logger.debug('Retrieving account_id from account_name')
    header = DEFAULT_HEADER
    header.update({"Authorization": session})
    # 2 options of search - if safe name not empty, add it to query, if not - search without it
    ...
    if rest_response.status_code == requests.codes.ok:
        ...
        logger.debug(f'No match for account: {account_name}')
        return False
    logger.error(f"Status code {rest_response.status_code}, received from REST service")
    raise Exception(f"Status code {rest_response.status_code}, received from REST service")
