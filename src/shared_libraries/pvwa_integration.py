...
class pvwa_integration:
    ...
    def call_rest_api_get(self, url, header):
        ...
        self.header = header
        try:
            restResponse = requests.get(self.url, timeout=30, verify=certificate, headers=self.header)
        except Exception as e:
    ...
    def call_rest_api_delete(self, url, header):
        ...
        self.header = header
        try:
            response = requests.delete(self.url, timeout=30, verify=certificate, headers=self.header)
        except Exception as e:
            print(e)
            return None
        return response
    ...
    def call_rest_api_post(self, url, request, header):
        ...
        self.header = header
        try:
            restResponse = requests.post(self.url, data=self.request, timeout=30, verify=certificate, headers=self.header, stream=True)
        except Exception:
